...
function saveAssignments(list: LocalAssignment[]) {
  ...
  // Mirror to electron-store so events survive across app rebuilds / device moves.
  window.seniorPartner?.calendarSave?.(list)
}
...
const EVENT_STYLE: Record<CalEvent['color'], string> = {
  ...
  midterm:    'bg-tertiary text-on-tertiary',
      ...
      <div className="bg-surface-container-lowest w-full max-w-lg rounded-xl shadow-[var(--shadow-modal)] overflow-hidden border border-outline-variant/20">
        <div className="p-8">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={onClose}
                className="flex-1 bg-surface-container-lowest text-primary border border-outline-variant/30 rounded-full py-3 font-bold text-sm hover:bg-surface-container-low transition-all">Cancel</button>
              <button type="submit"
...
function DayDetailModal({ dateStr, events, assignments, onEdit, onDelete, onComplete, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-surface-container-lowest w-full max-w-md rounded-xl shadow-[var(--shadow-modal)] border border-outline-variant/20 overflow-hidden" onClick={(e) => e.stopPropagation()}>
...
export function Calendar() {
  ...
  // Hydrate from electron-store (authoritative on desktop), falling back to localStorage.
  useEffect(() => {
    window.seniorPartner?.calendarGet?.().then((res) => {
      if (res?.success && Array.isArray(res.events) && res.events.length) {
        setAssignments(res.events as LocalAssignment[])
      }
    }).catch(() => { /* web dev — localStorage only */ })
  }, [])
