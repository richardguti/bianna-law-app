...
/* Schema hint handed to the main-process formatter / repair pass. */
const SYLLABUS_SCHEMA_HINT = `A single valid JSON object (no markdown fences):
{
  "courseName": "string",
  "professor": "string",
  "semester": "Season YYYY",
  "assignments": [ { "title": "string", "type": "midterm|final|quiz|assignment|reading", "dueDate": "YYYY-MM-DD" } ],
  "cases": [ { "week": 1, "date": "YYYY-MM-DD", "caseName": "string", "doctrineArea": "string", "isProfessorHook": false } ]
}
Every week in the source schedule must appear in "cases" — do not summarise or skip weeks.
Each "cases" entry must carry week, date and caseName.`
...
export function ReadingTracker() {
  ...
  async function parseSyllabus() {
    ...
    try {
      // Route through the Electron staged pipeline: R1 reasons, deepseek-chat
      // guarantees the JSON shape, and main-process validation rejects a truncated
      // week list (a half-parsed schedule is worse than a clear error).
      let rawText = ''
      const sp = window.seniorPartner

      if (sp?.aiStructured) {
        const result = await sp.aiStructured({
          messages: [
            { role: 'system', content: PARSE_SYSTEM },
            { role: 'user',   content: syllabusText.slice(0, 12000) },
          ],
          maxTokens: 16384,
          requiredKeys: ['courseName', 'cases'],
          validate: 'readingList',
          schemaHint: SYLLABUS_SCHEMA_HINT,
        })
        if (!result.success) throw new Error(result.error ?? 'Syllabus parsing failed.')
        rawText = result.text ?? ''
      } else {
        // Browser dev fallback (no Electron bridge available).
        const res = await fetch(CLAUDE_URL, {
          ...
          body: JSON.stringify({
            ...
            messages: [
              { role: 'system', content: PARSE_SYSTEM },
              { role: 'user', content: syllabusText.slice(0, 8000) },
            ],
          }),
        })
      ...
      if (start < 0 || end < start) throw new Error('DeepSeek returned an unexpected format — could not parse syllabus JSON.')
      ...
      // Push course info to local long-term memory so all AI features know Bianna's schedule
      ;(window as any).seniorPartner?.memoryWrite?.({
