...
export function Sidebar() {
  ...
  const photo = profile.photoBase64 || profileIcon
  ...
  return (
    <aside className="h-screen w-64 fixed left-0 top-0 bg-surface-container-low flex flex-col py-6 px-4 gap-2 z-40 border-r border-outline-variant/20">
      ...
      >
        <span className="material-symbols-outlined text-xl text-error">mail</span>
      ...
      <div className="pt-4 border-t border-outline-variant/10 space-y-1">
        ...
        <div className="flex items-center gap-3 px-3 py-2 text-xs text-on-surface-variant">
          ...
          <div>
            <p className="font-semibold text-on-surface text-xs">{profile.name || 'Bianna'}</p>
            <p className="text-[10px] tracking-wide opacity-60">{[profile.school, profile.year].filter(Boolean).join(' · ') || 'St. Thomas · 1L'}</p>
          </div>
        </div>
      </div>
    </aside>
