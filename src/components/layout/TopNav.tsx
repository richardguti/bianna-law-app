...
type Diag = {
  ...
  icmIndexed:       number
  icmByDiscipline?: Record<string, number>
  icmByDocType?:    Record<string, number>
  homedir?:         string
  freeMemMb:        number
}
...
export function TopNav() {
  ...
  async function openTroubleshoot() {
    ...
    setAnalysis(res?.response ?? 'Could not reach DeepSeek R1 for analysis.')
    setAnalyzing(false)
  }
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              ...
              {diag && (
                <div className="rounded-xl bg-surface-container p-4">
                  <DiagRow ok={diag.apiKeyPresent}        label="DeepSeek API Key"       detail={diag.apiKeyPresent ? 'Configured' : 'Missing — go to Settings'} />
                  <DiagRow ok={diag.notionKeyPresent}     label="Notion Key"             detail={diag.notionKeyPresent ? 'Configured' : 'Optional'} />
                  <DiagRow ok={diag.icmIndexed > 0}       label="Study Corpus"           detail={`${diag.icmIndexed} file${diag.icmIndexed !== 1 ? 's' : ''} indexed`} />
                  ...
                  <div className="pt-2 mt-1">
                    ...
                    <p className="text-[10px] text-on-surface-variant/50 font-label truncate mt-0.5" title={diag.biannaDir}>
                      {diag.biannaDir}
                    </p>
                  </div>
                </div>
