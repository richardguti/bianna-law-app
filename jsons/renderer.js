...
btnKeySave.addEventListener('click', async () => {
  ...
  try {
    if (deepseekKey) {
      const r = await window.seniorPartner.apiKeySave(deepseekKey);
      if (r.success) {
        setKeyStatus('DeepSeek key saved.', 'success');
        apiKeyInput.value = '';
      } else {
        setKeyStatus(r.error || 'Failed to save DeepSeek key.', 'error');
      }
    }
    if (notionKey) {
...
btnKeyClear.addEventListener('click', async () => {
  if (!confirm('Clear your saved DeepSeek API key?')) return;
  await window.seniorPartner.apiKeyClear();
  setKeyStatus('DeepSeek key cleared.', 'success');
